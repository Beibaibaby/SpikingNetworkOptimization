...
do
    ...
    mcc -m monkey_simulation.m -a src -a src/fa_Yu -o monkey_${monkey}

    # Create a temporary SLURM script for this job
    cat << EOF > temp_monkey_${monkey}.sh
#!/bin/bash
#SBATCH --job-name=monkey${monkey}
...
#SBATCH --mem=50GB
#SBATCH --output=/gpfs/data/doiron-lab/draco/out_monkey/monkey${monkey}.%j.txt
#SBATCH --error=/gpfs/data/doiron-lab/draco/out_monkey/monkey${monkey}.%j.err
