...
for i = 1:1
    ...
    [obj_configs,optimization_opt]=generate_config('is_spatial', 1,... % =1 using the SBN, =0 using the CBN.
                                                   ...
                                                   'real_data_name', strcat('matt_',string(i)),... % name pattern for the target stats, must be placed under /data.
                                                   'filename', strcat('matt_output_',string(i))); % name pattern for the logging files, will be placed under /results. Note: filename must end with "_bo_output_" for the recyling mechanism to identify all concurrent files
