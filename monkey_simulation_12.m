function monkey_simulation_12(monkey_number)
...
[obj_configs, optimization_opt] = generate_config('is_spatial', 1, ... % =1 using the SBN, =0 using the CBN.
    ...
    'filename', sprintf('monkey_newout_%s', monkey_num), ... % name pattern for the logging files, will be placed under /results. Note: filename must end with "_bo_output_" for the recyling mechanism to identify all concurrent files
    'root', './data_monkey_new/', ... % root directory for loading data
    'save_root', './results_monkey_new/', ... % root directory for saving results
